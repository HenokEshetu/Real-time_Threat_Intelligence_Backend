...
const createRelationship = (
  ...
  if (!validRelationships.get(sourceType)?.has(relationshipType)) {
    console.warn(
      `Invalid relationship type ${relationshipType} for source ${sourceType}: ${sourceId}`,
    );
    throw new Error(
      `Invalid relationship type: ${relationshipType} not allowed for ${sourceType}`,
    );
  }
  ...
  if (!validTargets.get(relationshipType)?.has(targetType)) {
    console.warn(
      `Invalid target ${targetType} for relationship ${relationshipType} from ${sourceType}: ${sourceId} -> ${targetId}`,
    );
    throw new Error(
      `Invalid relationship target: ${relationshipType} cannot target ${targetType}`,
    );
  }
  ...
  const relationship: GenericStixObject = {
    id: generateStixId(
      'relationship',
      `${sourceId}_${relationshipType}_${targetId}`,
    ),
    type: 'relationship',
    spec_version: STIX_VERSION,
    ...
    target_ref: targetId,
    description:
      description || `Relationship of type ${relationshipType} from MISP data`,
  ...
  console.log(
    `Created relationship: ${sourceType} (${sourceId}) ${relationshipType} ${targetType} (${targetId})`,
  );
  return relationship;
};
...
const createSighting = (
  ...
  id: generateStixId(
    'sighting',
    `${sightingOf}_sighted_${whereSighted.join('_')}`,
  ),
  type: 'sighting',
  spec_version: STIX_VERSION,
...
const mapMispRelationshipToStix = (
  mispRel: string,
  sourceType: string,
  targetType: string,
): string | null => {
  ...
  try {
    if (ip.includes('.')) {
      ...
      return parts.every((part) => {
        ...
        return (
          !isNaN(num) && num >= 0 && num <= 255 && part.match(/^(0|[1-9]\d*)$/)
        );
      });
    }
    if (ip.includes(':')) {
...
const MISP_TO_STIX_MAPPING: Record<
  string,
  { type: StixType; isSDO?: boolean }
> = {
  md5: { type: 'file' },
  sha1: { type: 'file' },
  sha256: { type: 'file' },
...
function* processMispObject(
  mispObject: any,
  eventTimestamp: number,
): Generator<GenericStixObject, void, unknown> {
  console.log(
    `Processing MISP object: ${JSON.stringify(mispObject, null, 2).substring(0, 500)}`,
  );
  ...
  try {
    if (!mispObject.name || !mispObject.uuid) {
      console.warn(
        `Skipping MISP object with missing name or uuid: ${JSON.stringify(mispObject)}`,
      );
      return;
    }
    ...
    switch (mispObject.name) {
      case 'file':
        const fileObj: GenericStixObject = {
          ...
          type: 'file',
          spec_version: STIX_VERSION,
          name: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'filename',
          )?.value,
          hashes: {
            MD5: mispObject.attributes?.find(
              (a: any) => a.object_relation === 'md5',
            )?.value,
            'SHA-1': mispObject.attributes?.find(
              (a: any) => a.object_relation === 'sha1',
            )?.value,
            'SHA-256': mispObject.attributes?.find(
              (a: any) => a.object_relation === 'sha256',
            )?.value,
            'SHA-512': mispObject.attributes?.find(
              (a: any) => a.object_relation === 'sha512',
            )?.value,
            SSDEEP: mispObject.attributes?.find(
              (a: any) => a.object_relation === 'ssdeep',
            )?.value,
          },
          size:
            parseInt(
              mispObject.attributes?.find(
                (a: any) => a.object_relation === 'size-in-bytes',
              )?.value,
              10,
            ) || undefined,
          created: moment(timestamp * 1000).toISOString(),
      ...
      case 'email':
        const emailMessageObj: GenericStixObject = {
          ...
          type: 'email-message',
          spec_version: STIX_VERSION,
          subject: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'subject',
          )?.value,
          body: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'message-body',
          )?.value,
          from_ref: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'from' && isValidEmail(a.value),
          )?.value
            ? generateStixId('email-addr', mispObject.uuid + '-from')
            : undefined,
          to_refs: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'to' && isValidEmail(a.value),
          )?.value
            ? [generateStixId('email-addr', mispObject.uuid + '-to')]
            : undefined,
          date: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'send-date',
          )?.value
            ? moment(
                mispObject.attributes?.find(
                  (a: any) => a.object_relation === 'send-date',
        ...
        if (emailMessageObj.from_ref) {
          const fromAddrObj: GenericStixObject = {
            ...
            type: 'email-addr',
            spec_version: STIX_VERSION,
            value: mispObject.attributes?.find(
              (a: any) => a.object_relation === 'from',
            )?.value,
            created: moment(timestamp * 1000).toISOString(),
            modified: moment(timestamp * 1000).toISOString(),
            labels: ['misp-object', 'email'],
      ...
      case 'network-traffic':
        const srcIp = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'ip-src',
        )?.value;
        const dstIp = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'ip-dst',
        )?.value;
        const networkTrafficObj: GenericStixObject = {
          ...
          spec_version: STIX_VERSION,
          src_ref:
            srcIp && isValidIp(srcIp)
              ? generateStixId(
                  srcIp.includes(':') ? 'ipv6-addr' : 'ipv4-addr',
                  mispObject.uuid + '-src',
                )
              : undefined,
          dst_ref:
            dstIp && isValidIp(dstIp)
              ? generateStixId(
                  dstIp.includes(':') ? 'ipv6-addr' : 'ipv4-addr',
                  mispObject.uuid + '-dst',
                )
              : undefined,
          src_port:
            parseInt(
              mispObject.attributes?.find(
                (a: any) => a.object_relation === 'port-src',
              )?.value,
              10,
            ) || undefined,
          dst_port:
            parseInt(
              mispObject.attributes?.find(
                (a: any) => a.object_relation === 'port-dst',
              )?.value,
              10,
            ) || undefined,
          protocols: [
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'protocol',
            )?.value || 'tcp',
          ],
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'process':
        const processObj: GenericStixObject = {
          ...
          pid:
            parseInt(
              mispObject.attributes?.find(
                (a: any) => a.object_relation === 'pid',
              )?.value,
              10,
            ) || undefined,
          command_line: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'command-line',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'windows-registry-key':
        const registryKeyObj: GenericStixObject = {
          ...
          spec_version: STIX_VERSION,
          key: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'key',
          )?.value,
          values:
            mispObject.attributes
              ?.filter((a: any) => a.object_relation === 'value')
      ...
      case 'url':
        const urlValue = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'url',
        )?.value;
      ...
      case 'domain-name':
        const domainValue = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'domain',
        )?.value;
        const ipValue = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'ip',
        )?.value;
        if (
          domainValue &&
          ...
          const ipObj: GenericStixObject = {
            id: generateStixId(
              ipValue.includes(':') ? 'ipv6-addr' : 'ipv4-addr',
              mispObject.uuid + '-ip',
            ),
      ...
      case 'autonomous-system':
        const asNumber = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'asn',
        )?.value;
        if (asNumber) {
      ...
      case 'user-account':
        const userAccountObj: GenericStixObject = {
          ...
          type: 'user-account',
          spec_version: STIX_VERSION,
          user_id: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'username',
          )?.value,
          account_type:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'account-type',
            )?.value || 'unknown',
          display_name: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'display-name',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'x509':
        const x509Obj: GenericStixObject = {
          ...
          spec_version: STIX_VERSION,
          issuer: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'issuer',
          )?.value,
          serial_number: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'serial-number',
          )?.value,
          hashes: {
            MD5: mispObject.attributes?.find(
              (a: any) => a.object_relation === 'md5',
      ...
      case 'software':
        const softwareObj: GenericStixObject = {
          ...
          version: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'version',
          )?.value,
          vendor: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'vendor',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'directory':
        const directoryObj: GenericStixObject = {
          ...
          type: 'directory',
          spec_version: STIX_VERSION,
          path: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'path',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'mac-address':
        const macValue = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'mac-address',
        )?.value;
      ...
      case 'artifact':
        const artifactObj: GenericStixObject = {
          ...
          type: 'artifact',
          spec_version: STIX_VERSION,
          payload_bin: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'data',
          )?.value,
          mime_type:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'mime-type',
      ...
      case 'vulnerability':
        const vulnerabilityObj: GenericStixObject = {
          ...
          spec_version: STIX_VERSION,
          name:
            mispObject.attributes?.find((a: any) => a.object_relation === 'id')
              ?.value || 'Unknown Vulnerability',
          description: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'description',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'course-of-action':
        const courseOfActionObj: GenericStixObject = {
          ...
          name:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'name',
            )?.value || 'Unknown Course of Action',
          description: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'description',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
      ...
      case 'location':
        const locationObj: GenericStixObject = {
          ...
          country: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'country',
          )?.value,
          city: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'city',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'indicator':
        const indicatorValue = mispObject.attributes?.find(
          (a: any) => a.object_relation === 'pattern',
        )?.value;
        if (!indicatorValue) {
          console.warn(
            `Indicator object missing pattern: ${JSON.stringify(mispObject)}`,
          );
        }
      ...
      case 'note':
        const noteObj: GenericStixObject = {
          ...
          content:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'content',
      ...
      case 'opinion':
        const opinionObj: GenericStixObject = {
          ...
          type: 'opinion',
          spec_version: STIX_VERSION,
          explanation: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'explanation',
          )?.value,
          opinion:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'opinion',
            )?.value || 'unknown',
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
      ...
      case 'malware-analysis':
        const malwareAnalysisObj: GenericStixObject = {
          ...
          product:
            mispObject.attributes?.find(
              (a: any) => a.object_relation === 'product',
            )?.value || 'Unknown',
          result: mispObject.attributes?.find(
            (a: any) => a.object_relation === 'result',
          )?.value,
          created: moment(timestamp * 1000).toISOString(),
          modified: moment(timestamp * 1000).toISOString(),
    ...
    console.error(
      `Error processing MISP object ${mispObject.uuid || 'unknown'}: ${(error as Error).message}`,
    );
  }
}

export function* processMispData(
  raw: any,
): Generator<GenericStixObject, void, unknown> {
  try {
    console.log(
      `Processing MISP raw data: ${JSON.stringify(raw, null, 2).substring(0, 1000)}`,
    );
    ...
    console.log(
      `Attributes count: ${attributes.length}, Objects count: ${objects.length}, ObjectReferences count: ${objectReferences.length}`,
    );
    ...
    const validTargets = new Map<string, Set<string>>([
      ...
      [
        'uses',
        new Set(['attack-pattern', 'infrastructure', 'malware', 'tool']),
      ],
      ['attributed-to', new Set(['intrusion-set', 'threat-actor', 'identity'])],
      ['compromises', new Set(['infrastructure'])],
      ['originates-from', new Set(['location'])],
      ...
      for (const galaxy of event.Galaxy) {
        if (galaxy.GalaxyCluster) {
          for (const cluster of galaxy.GalaxyCluster) {
            ...
            if (
              cluster.type.includes('attack-pattern') ||
              cluster.type.includes('mitre-attack-pattern')
            )
            ...
            else if (cluster.type.includes('threat-actor'))
    ...
    if (isEvent) {
      const tlpMarking = createTLPMarking('white', eventTimestamp);
      const reportId = generateStixId(
        'report',
        event.uuid || uuidv5('misp-event', NAMESPACE),
      );
      ...
      const reportTypes = event.Tag
        ? event.Tag.filter((t: any) => t.name.startsWith('report-type:'))
      ...
      report = {
        ...
        description:
          event.extended_info ||
          event.info ||
          'Report generated from MISP event',
        ...
        confidence: event.analysis
          ? mapMispAnalysisToConfidence(event.analysis)
          : undefined,
        lang: event.lang || 'en',
    ...
    for (const attr of attributes) {
      try {
        console.log(
          `Processing MISP attribute: ${JSON.stringify(attr, null, 2)}`,
        );
        const mapping =
          MISP_TO_STIX_MAPPING[attr.type] || MISP_TO_STIX_MAPPING.default;
        ...
        if (!value) {
          console.warn(
            `Missing value for MISP attribute: ${JSON.stringify(attr)}`,
          );
          const tlpMarking = createTLPMarking('white', timestamp);
        ...
        const compositeValues = attr.type.includes('|')
        ...
        if (isSDO) {
          ...
          if (stixType === 'indicator') {
            if (!value) {
              console.warn(
                `Skipping indicator with empty value: ${JSON.stringify(attr)}`,
              );
              objectToAdd = {
                ...baseObject,
                type: 'observed-data',
              ...
              objectToAdd.pattern =
                attr.pattern ||
                `[${attr.type}:value = '${value.replace(/'/g, "\\'")}']`;
            ...
            objectToAdd.malware_types =
              category === 'Payload delivery' ? ['trojan'] : ['unknown'];
            ...
            objectToAdd.description =
              attr.comment || `Course of Action ${value}`;
          } else if (stixType === 'identity') {
            objectToAdd.identity_class =
              category === 'Person' ? 'individual' : 'organization';
          ...
          switch (stixType) {
            ...
            case 'ipv6-addr':
              ...
              if (!isValidIp(ipValue)) {
                console.warn(
                  `Invalid IP address for attribute ${attrUuid}: ${ipValue}`,
                );
                objectToAdd = {
                  ...baseObject,
                  type: 'observed-data',
            ...
            case 'domain-name':
              ...
              const resolvesToRefs =
                compositeValues?.[1] && isValidIp(compositeValues[1])
                  ? [
                      generateStixId(
                        compositeValues[1].includes(':')
                          ? 'ipv6-addr'
                          : 'ipv4-addr',
                        compositeValues[1],
                      ),
            ...
            case 'file':
              ...
              const hashType =
                hashTypeMap[attr.type.split('|')[0]] || 'SHA-256';
              const fileName =
                compositeValues?.[0] ||
                (attr.type === 'filename' ? value : undefined);
            ...
            case 'network-traffic':
              ...
              objectToAdd = {
                ...
                dst_ref:
                  ip && isValidIp(ip)
                    ? generateStixId(
                        ip.includes(':') ? 'ipv6-addr' : 'ipv4-addr',
                        ip,
                      )
                    : undefined,
            ...
            case 'user-account':
              objectToAdd = {
                ...
                user_id: value,
                account_type:
                  category === 'Financial fraud' ? 'financial' : 'unknown',
              };
              break;
            case 'identity':
              objectToAdd = {
            ...
            default:
              console.warn(
                `Unhandled STIX type ${stixType} for attribute ${attrUuid}`,
              );
              objectToAdd = {
                ...baseObject,
                type: 'observed-data',
        ...
        if (objectToAdd) {
          ...
          if (toIds && STIX_SCO_TYPES.includes(stixType)) {
            const indicatorObj: GenericStixObject = {
              ...
              pattern:
                objectToAdd.pattern ||
                `[${stixType}:value = '${value.replace(/'/g, "\\'")}']`,
        ...
        console.error(
          `Error processing MISP attribute ${attr.uuid || 'unknown'}: ${(error as Error).message}`,
        );
      }
    }
    ...
    for (const obj of objects) {
      try {
        ...
        console.error(
          `Error processing MISP object ${obj.uuid || 'unknown'}: ${(error as Error).message}`,
        );
      }
    }
    ...
    for (const ref of objectReferences) {
      ...
      if (sourceObj && targetObj) {
        const mappedType = mapMispRelationshipToStix(
          relationshipType,
          sourceObj.type,
          targetObj.type,
        );
        relationshipType =
          mappedType &&
          validRelationships.get(sourceObj.type)?.has(mappedType) &&
          validTargets.get(mappedType)?.has(targetObj.type)
            ? mappedType
            : 'related-to';

        const relationshipId = generateStixId(
          'relationship',
          `${sourceObj.id}_${relationshipType}_${targetObj.id}`,
        );
    ...
    for (const sourceObj of Object.values(stixObjectMap)) {
      ...
      for (const targetObj of Object.values(stixObjectMap)) {
        ...
        for (const relType of allowedRelationshipTypes) {
          ...
          if (
            sourceObj.type === 'malware' &&
            targetObj.type === 'file' &&
            sourceObj.labels?.includes('dropper')
          ) {
            inferredType = 'drops';
          } else if (
            sourceObj.type === 'malware' &&
            targetObj.type === 'file' &&
            targetObj.labels?.includes('delivered')
          ) {
            inferredType = 'delivers';
          } else if (
            sourceObj.type === 'threat-actor' &&
            targetObj.type === 'campaign'
          ) {
            inferredType = 'attributed-to';
          } else if (
            sourceObj.type === 'indicator' &&
            relType === 'indicates'
          ) {
            // Explicitly validate that target is a valid SDO for 'indicates'
            const validIndicatesTargets = new Set([
              'attack-pattern',
              'campaign',
              'infrastructure',
              'intrusion-set',
            ...
            if (validIndicatesTargets.has(targetObj.type)) {
              ...
              console.warn(
                `Skipping invalid 'indicates' relationship from indicator to ${targetObj.type}: ${sourceObj.id} -> ${targetObj.id}`,
              );
              continue;
            }
          }
          ...
          if (inferredType) {
            const relationshipId = generateStixId(
              'relationship',
              `${sourceObj.id}_${inferredType}_${targetObj.id}`,
            );
            if (!createdRelationships.has(relationshipId)) {
              const relationship = createRelationship(
                sourceObj.id,
              ...
              console.log(
                `Creating inferred relationship: ${sourceObj.type} (${sourceObj.id}) ${inferredType} ${targetObj.type} (${targetObj.id})`,
              );
              yield relationship;
              createdRelationships.add(relationshipId);
              if (report) report.object_refs.push(relationship.id);
    ...
    for (const sighting of event.Sighting || []) {
      ...
      if (sightedObject) {
        const orgIdentity: GenericStixObject = Object.values(
          stixObjectMap,
        ).find(
          (o) => o.type === 'identity' && o.identity_class === 'organization',
        ) || {
...
function* processAlienVaultOTXPulse(
  raw: any,
): Generator<GenericStixObject, void, unknown> {
  try {
    console.log(
      `[alienVaultOTX] Starting processing of AlienVault OTX pulse data: ${JSON.stringify(raw, null, 2).substring(0, 1000)}`,
    );
    if (
      !raw ||
      typeof raw !== 'object' ||
      ...
      console.warn(
        `[alienVaultOTX] Invalid AlienVault OTX pulse data: ${JSON.stringify(raw, null, 2).substring(0, 500)}`,
      );
      return;
    }
    ...
    const stixObjectMap: Record<string, GenericStixObject> = {};
    const pulseId =
      raw.id || raw.pulse_id || uuidv5('alienvault-otx-pulse', NAMESPACE);
    ...
    const externalReferences =
      raw.references && Array.isArray(raw.references)
    ...
    console.log(
      `[alienVaultOTX] Created TLP marking for pulse ${pulseId}: TLP ${tlp}, ID ${tlpMarking.id}`,
    );
    yield tlpMarking;
    ...
    const determineReportTypes = () => {
      ...
      if (raw.indicators.length > 5) {
        // Arbitrary threshold for "indicator-heavy"
        types.add('indicator');
      }
    ...
    console.log(
      `[alienVaultOTX] Created STIX report for pulse ${pulseId}: ${report.id}, ${report.name}`,
    );
    yield report;
    ...
    for (const indicator of raw.indicators) {
      try {
        console.log(
          `[alienVaultOTX] Processing indicator for pulse ${pulseId}: ${JSON.stringify(indicator, null, 2).substring(0, 500)}`,
        );
        if (
          !indicator.type ||
          !indicator.indicator ||
          ...
          console.warn(
            `[alienVaultOTX] Skipping invalid or inactive indicator for pulse ${pulseId}: ${JSON.stringify(indicator, null, 2).substring(0, 500)}`,
          );
          skippedIndicators++;
          continue;
        }
        ...
        const baseObj: GenericStixObject = {
          ...
          created: indicator.created
            ? moment(indicator.created).toISOString()
            : created,
          modified,
          labels: [
            'alienvault-otx',
        ...
        switch (stixType) {
          ...
          case 'ipv6-addr':
            if (
              ...
              console.warn(
                `[alienVaultOTX] Invalid IP address for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          case 'domain-name':
            if (!value.match(/^(?!-)(?:[a-zA-Z0-9-]{1,63}\.)+[a-zA-Z]{2,}$/)) {
              console.warn(
                `[alienVaultOTX] Invalid domain name for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          case 'url':
            try {
              ...
              console.warn(
                `[alienVaultOTX] Invalid URL for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          case 'email-addr':
            if (!value.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
              console.warn(
                `[alienVaultOTX] Invalid email address for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          case 'file':
            ...
            if (!value.match(/^[0-9a-fA-F]+$/)) {
              console.warn(
                `[alienVaultOTX] Invalid hash value for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          case 'vulnerability':
            if (!value.startsWith('CVE-')) {
              console.warn(
                `[alienVaultOTX] Invalid CVE identifier for pulse ${pulseId}, indicator ${indicatorId}: ${value}`,
              );
              skippedIndicators++;
              continue;
            }
          ...
          default:
            ...
            console.log(
              `[alienVaultOTX] Created fallback observed-data for pulse ${pulseId}, indicator ${indicatorId}: ${stixObj.id}`,
            );
            break;
        }
        ...
        if (stixObj) {
          console.log(
            `[alienVaultOTX] Created STIX object for pulse ${pulseId}, indicator ${indicatorId}: ${stixType} (${stixObj.id})`,
          );
          yield stixObj;
        ...
        console.error(
          `[alienVaultOTX] Error processing indicator ${indicator.id || 'unknown'} for pulse ${pulseId}: ${(error as Error).message}`,
          {
    ...
    console.log(
      `[alienVaultOTX] Completed processing pulse ${pulseId}: ${processedIndicators} indicators processed, ${skippedIndicators} skipped, ${Object.keys(stixObjectMap).length} STIX objects created`,
    );
  } catch (error) {
    console.error(
      `[alienVaultOTX] Error processing AlienVault OTX pulse ${this.pulseId}: ${(error as Error).message}`,
      {
        raw: JSON.stringify(raw, null, 2).substring(0, 500),
      },
    );
...
function* processHybridAnalysis(
  raw: any,
): Generator<GenericStixObject, void, unknown> {
  ...
  try {
    console.log(
      `Processing Hybrid Analysis data: ${JSON.stringify(raw, null, 2).substring(0, 1000)}`,
    );
    if (!raw || typeof raw !== 'object') {
      console.warn(
        `Invalid Hybrid Analysis data: ${JSON.stringify(raw, null, 2).substring(0, 500)}`,
      );
      return;
    }
    ...
    const stixObjectMap: Record<string, GenericStixObject> = {};
    const createdRelationships: Set<string> = new Set();
    const timestamp = raw.analysis_start_time
      ? moment(raw.analysis_start_time).unix()
      : moment().unix();
    const created = moment(timestamp * 1000).toISOString();
    ...
    const extractedFiles = Array.isArray(raw.extracted_files)
      ? raw.extracted_files
      : [];
    const geolocation = Array.isArray(raw.hosts_geolocation)
      ? raw.hosts_geolocation
      : [];
    const reportUrl = raw.report_url
      ? `https://www.hybrid-analysis.com${raw.report_url}`
      : null;
    const tags = Array.isArray(raw.tags)
      ? raw.tags.map((tag: string) => tag.toLowerCase())
      : [];
    ...
    const description = [
      ...
      processes.length > 0
        ? `Processes: ${processes.map((p: any) => p.name).join(', ')}`
        : null,
    ...
    if (raw.sha256 || raw.md5 || jobId) {
      externalReferences.push({
        ...
        url:
          reportUrl ||
          `https://www.hybrid-analysis.com/sample/${raw.sha256 || raw.md5 || jobId}`,
      ...
      labels: [
        ...
        ...Array.from(
          new Set(
            geolocation
              .filter(
                (geo: any) => geo.country && typeof geo.country === 'string',
              )
    ...
    if (raw.sha256 || raw.md5 || raw.sha1) {
      ...
      if (hashes.MD5 || hashes['SHA-1'] || hashes['SHA-256']) {
        primaryFileObj = {
          ...
          hashes,
          pattern: hashes['SHA-256']
            ? `[file:hashes.'SHA-256' = '${hashes['SHA-256']}']`
            : undefined,
          pattern_type: 'stix',
        };
        ...
        const indicatorObj: GenericStixObject = {
          ...
          type: 'indicator',
          pattern:
            primaryFileObj.pattern ||
            `[file:hashes.'SHA-256' = '${hashes['SHA-256'] || 'unknown'}']`,
          pattern_type: 'stix',
          valid_from: created,
    ...
    if (isUrlAnalysis && submitName && isValidUrl(submitName)) {
      ...
      if (
        primaryFileObj &&
        this.validRelationships.get('file')?.has('downloaded-from')
      ) {
        const relationship = createRelationship(
          primaryFileObj.id,
          urlObj.id,
    ...
    for (const domain of domains) {
      try {
        if (isValidDomain(domain)) {
          ...
          if (
            primaryFileObj &&
            this.validRelationships.get('domain-name')?.has('communicates-with')
          ) {
            const relationship = createRelationship(
              domainObj.id,
              primaryFileObj.id,
        ...
        console.error(
          `Error processing domain ${domain}: ${(error as Error).message}`,
        );
      }
    }
    ...
    const uniqueIps = new Set<string>([
      ...hosts,
      ...(Array.isArray(raw.et_alerts)
        ? raw.et_alerts
            .map((alert: any) => alert.destination_ip)
            .filter(Boolean)
        : []),
    ]);
        ...
        if (isValidIp(ip)) {
          ...
          for (const domain of domains) {
            if (
              ...
              this.validRelationships.get('domain-name')?.has('resolves-to')
            ) {
              const relationship = createRelationship(
          ...
          if (
            primaryFileObj &&
            this.validRelationships.get(ipObj.type)?.has('communicates-with')
          ) {
            const relationship = createRelationship(
              ipObj.id,
              primaryFileObj.id,
    ...
    for (const file of extractedFiles) {
      try {
        ...
        if (
          extractedHashes.MD5 ||
          extractedHashes['SHA-1'] ||
          extractedHashes['SHA-256']
        ) {
          const extractedFileObj: GenericStixObject = {
            ...
            id: generateStixId(
              ...
              file.sha256 ||
                file.md5 ||
                uuidv5(file.name || 'unknown', NAMESPACE),
            ),
            type: 'file',
            hashes: extractedHashes,
            name: file.name || undefined,
            description:
              file.description || `Extracted file: ${file.name || 'unknown'}`,
            labels: [
              ...baseObj.labels,
              `type:${file.type_tags?.join(',') || 'unknown'}`,
            ],
            pattern: extractedHashes['SHA-256']
              ? `[file:hashes.'SHA-256' = '${extractedHashes['SHA-256']}']`
              : undefined,
            pattern_type: 'stix',
          };
          ...
          if (
            primaryFileObj &&
            this.validRelationships.get('file')?.has('derived-from')
          ) {
            const relationship = createRelationship(
              extractedFileObj.id,
              primaryFileObj.id,
          ...
          const indicatorObj: GenericStixObject = {
            ...
            type: 'indicator',
            pattern:
              extractedFileObj.pattern ||
              `[file:hashes.'SHA-256' = '${extractedHashes['SHA-256'] || 'unknown'}']`,
            pattern_type: 'stix',
            valid_from: created,
        ...
        console.error(
          `Error processing extracted file ${file.name || 'unknown'}: ${(error as Error).message}`,
        );
      }
    }
    ...
    for (const process of processes) {
      try {
        if (process.sha256 && isValidSha256(process.sha256)) {
          ...
          if (
            primaryFileObj &&
            this.validRelationships.get('file')?.has('executed-by')
          ) {
            const relationship = createRelationship(
              processFileObj.id,
              primaryFileObj.id,
        ...
        console.error(
          `Error processing process ${process.name || 'unknown'}: ${(error as Error).message}`,
        );
      }
    }
    ...
    if (threatScore > 0) {
      ...
      for (const objId of Object.keys(stixObjectMap)) {
        ...
        if (
          STIX_SCO_TYPES.includes(obj.type) &&
          !objId.startsWith('indicator-')
        ) {
    ...
    console.log(
      `Processed Hybrid Analysis sample ${jobId}, yielding STIX objects incrementally`,
    );
  } catch (error) {
...
export const objectMappers: Record<
  string,
