...
export const indicatorMappers: Record<
  string,
  (raw: any) => GenericStixObject | GenericStixObject[] | null
> = {
  hybridAnalysis: (
    raw: any,
  ): GenericStixObject | GenericStixObject[] | null => {
    try {
      // Validate input
      ...
      const created = raw.analysis_start_time
        ? new Date(raw.analysis_start_time).toISOString()
        : new Date().toISOString();
      const modified = raw.analysis_start_time
        ? new Date(raw.analysis_start_time).toISOString()
        : created;
      ...
      if (raw.sha256 || raw.md5 || raw.sha1) {
        ...
        > = {
          MD5:
            typeof raw.md5 === 'string' && /^[0-9a-fA-F]{32}$/.test(raw.md5)
              ? raw.md5
              : undefined,
          'SHA-1':
            typeof raw.sha1 === 'string' && /^[0-9a-fA-F]{40}$/.test(raw.sha1)
              ? raw.sha1
              : undefined,
          'SHA-256':
            typeof raw.sha256 === 'string' &&
            /^[0-9a-fA-F]{64}$/.test(raw.sha256)
              ? raw.sha256
              : undefined,
          'SHA-512': undefined,
        };
        if (hashes['MD5'] || hashes['SHA-1'] || hashes['SHA-256']) {
          ...
          console.warn(
            `Invalid hash values in Hybrid Analysis data: ${JSON.stringify(raw, null, 2).substring(0, 500)}`,
          );
        }
      }
      ...
      domains.forEach((domain: string) => {
        if (
          ...
          domain.match(/^(?!-)(?:[a-zA-Z0-9-]{1,63}\.)+[a-zA-Z]{2,}$/)
        ) {
      ...
      extractedFiles.forEach((file: any) => {
        if (file.sha256 || file.md5 || file.sha1) {
          const extractedHashes: Record<
            'MD5' | 'SHA-1' | 'SHA-256' | 'SHA-512',
            string | undefined
          > = {
            MD5:
              typeof file.md5 === 'string' && /^[0-9a-fA-F]{32}$/.test(file.md5)
                ? file.md5
                : undefined,
            'SHA-1':
              typeof file.sha1 === 'string' &&
              /^[0-9a-fA-F]{40}$/.test(file.sha1)
                ? file.sha1
                : undefined,
            'SHA-256':
              typeof file.sha256 === 'string' &&
              /^[0-9a-fA-F]{64}$/.test(file.sha256)
                ? file.sha256
                : undefined,
            'SHA-512': undefined,
          };
          if (
            ...
            console.warn(
              `Invalid hash values in extracted file: ${JSON.stringify(file, null, 2).substring(0, 500)}`,
            );
          }
        }
      });
      ...
      processes.forEach((process: any) => {
        if (process.sha256) {
          ...
          > = {
            ...
            'SHA-1': undefined,
            'SHA-256':
              typeof process.sha256 === 'string' &&
              /^[0-9a-fA-F]{64}$/.test(process.sha256)
                ? process.sha256
                : undefined,
            'SHA-512': undefined,
          };
          if (processHashes['SHA-256']) {
            ...
            console.warn(
              `Invalid hash in process: ${JSON.stringify(process, null, 2).substring(0, 500)}`,
            );
          }
        }
      });
      ...
      console.error(
        `Mapper error for Hybrid Analysis: ${(e as Error).message}`,
        {
  ...
  alienVaultOTX: (raw: any): GenericStixObject | GenericStixObject[] | null => {
    try {
      ...
      if (
        ...
        console.warn(
          `Invalid AlienVault OTX pulse data: ${JSON.stringify(raw, null, 2).substring(0, 500)}`,
        );
        return null;
      }
      ...
      raw.indicators.forEach((indicator: any) => {
        if (!indicator.type || !indicator.indicator) {
          console.warn(
            `Missing type or indicator in OTX indicator: ${JSON.stringify(indicator, null, 2).substring(0, 500)}`,
          );
          return;
        }
        ...
        const baseObj: GenericStixObject = {
          ...
          created: indicator.created
            ? new Date(indicator.created).toISOString()
            : created,
          modified,
          labels: [
            'alienvault-otx',
          ...
          confidence,
          external_references:
            pulseReferences.length > 0 ? pulseReferences : undefined,
        ...
        switch (stixType) {
          ...
          case 'domain-name':
            ...
            if (indicator.type === 'hostname') {
              ...
              if (
                raw.indicators.some(
                  (ind: any) =>
                    ind.type === 'domain' && ind.indicator === parentDomain,
                )
              ) {
          ...
          case 'file':
            ...
            stixObj = {
              ...
              hashes: {
                ...
                'SHA-256':
                  indicator.type === 'FileHash-SHA256' ? value : undefined,
                'SHA-512': undefined,
          ...
          case 'vulnerability':
            ...
            stixObj = {
              ...
              description:
                ...
                `Vulnerability ${value} from AlienVault OTX`,
            };
            break;
          ...
          default:
            console.warn(
              `Unsupported STIX type: ${stixType} for OTX indicator: ${JSON.stringify(indicator, null, 2).substring(0, 500)}`,
            );
            return;
        }
      ...
      console.warn(
        `No valid STIX objects generated for OTX pulse: ${pulseId || 'unknown'}`,
      );
      return null;
    } catch (e) {
      console.error(
        `Mapper error for AlienVault OTX: ${(e as Error).message}`,
        {
          raw: JSON.stringify(raw, null, 2).substring(0, 500),
        },
      );
  ...
  misp: (raw: any): GenericStixObject | null => {
    try {
      ...
      if (!raw || (!raw.Event && !raw.Attribute && !raw.response?.Attribute)) {
        console.warn(
          `Invalid MISP data: missing Event or Attribute - ${JSON.stringify(raw)}`,
        );
        return null;
      }
      ...
      const attributes =
        (isEvent ? event.Attribute : raw.response?.Attribute) || [];
        ...
        external_references: [
          {
            ...
            url: event.uuid
              ? `http://localhost/events/${event.uuid}`
              : undefined,
      ...
      const galaxyClusters =
        event.GalaxyCluster?.map((cluster: any) => ({
          type: cluster.type.includes('mitre-attack-pattern')
            ? 'attack-pattern'
            : 'malware',
          name: cluster.value,
      ...
      if (isEvent && attributes.length === 0) {
        return {
          ...
          published: event.publish_timestamp
            ...
            : baseObj.created,
          object_refs: galaxyClusters.map(
            (gc: any) => `${gc.type}--${uuidv4()}`,
          ),
      ...
      if (attributes.length > 0) {
        ...
        const category =
          attribute.category ||
          raw.sane_defaults?.[attribute.type]?.default_category ||
          'Other';
        const toIds =
          attribute.to_ids ?? raw.sane_defaults?.[attribute.type]?.to_ids ?? 0;

        if (!value) {
          console.warn(
            `Missing value for MISP attribute: ${JSON.stringify(attribute)}`,
          );
          return null;
        }
        ...
        const effectiveType =
          toIds && stixType !== 'observed-data' ? 'indicator' : stixType;
        ...
        switch (effectiveType) {
          case 'file':
            ...
            const hashType =
              hashTypeMap[attribute.type.split('|')[0]] || 'SHA-256';
            const fileName =
              compositeValues?.[0] ||
              (attribute.type === 'filename' ? value : undefined);
            const hashes: Record<
              'MD5' | 'SHA-1' | 'SHA-256' | 'SHA-512',
              string
            > = {
          ...
          case 'ipv6-addr':
            ...
            if (!ipValue || ipValue === 'undefined') {
              console.warn(
                `Invalid IP value for MISP attribute: ${JSON.stringify(attribute)}`,
              );
              return null;
            }
            return {
          ...
          case 'network-traffic':
            ...
            return {
              ...
              dst_ref: ip
                ? `${ip.includes(':') ? 'ipv6-addr' : 'ipv4-addr'}--${uuidv4()}`
                : undefined,
      ...
      if (galaxyClusters.length > 0) {
        ...
        return {
          ...
          description: cluster.description,
          external_references: [
            ...baseObj.external_references,
            ...cluster.external_references,
          ],
      ...
      console.error(`MISP mapper error: ${(e as Error).message}`, {
