...
export class FeedIngesterService implements OnModuleInit {
    ...
    try {
      ...
      for await (const stixObject of objectGenerator) {
        ...
        if (processedCount % 100 === 0) {
          this.logger.log(
            `Processing progress for ${config.name}: ${processedCount} objects processed`,
          );
        }
      }

      this.logger.log(
        `Completed feed ${config.name}: ${successCount} stored, ${duplicateCount} duplicates, ${failedCount} failed in ${(Date.now() - startTime) / 1000}s`,
      );
    } catch (error) {
      this.logger.error(`Failed to process feed ${config.name}`, error);
    }
  ...
  private async processStixObject(
    ...
    if (!['marking-definition', 'relationship', 'sighting'].includes(type)) {
      if (!lookupValue) {
        this.logger.warn(`Skipping invalid object: no valid lookup value`, {
          objectId: obj.id,
        });
      ...
      if (existing) {
        this.logger.debug(`Duplicate object found: ${type} (${lookupValue})`, {
          objectId: obj.id,
        });
    ...
    try {
      ...
      const storedObject = await this.storeStixObject(
        stixResult.type,
        stixResult.input,
        obj,
      );
      if (
        stixResult.relationships?.length > 0 &&
        !['relationship', 'sighting'].includes(type)
      ) {
      ...
      if (stixResult.additionalObjects?.length > 0) {
        await Promise.allSettled(
          stixResult.additionalObjects.map(async (additionalObj) => {
            try {
              const result = await this.processStixObject(
                additionalObj,
                config,
              );
              if (result.success && !result.isDuplicate) {
                this.logger.debug(
                  `Processed additional object ${additionalObj.id}`,
                  {
                    type: additionalObj.type,
                    extensions: additionalObj.extensions
                      ? Object.keys(additionalObj.extensions)
                      : [],
                  },
                );
              }
              ...
              this.logger.error(
                `Failed to process additional object ${additionalObj.id}`,
                {
                  error: error.message,
                  type: additionalObj.type,
                },
  ...
  private mapObjectsWithConfig(
    object: any,
    config: FeedProviderConfig,
  ): GenericStixObject[] {
    try {
      const mapper =
        typeof config.objectMapper === 'string'
  ...
  private async transformToStixInput(
    ...
    if (
      !nonEnrichableTypes.includes(primaryObject.type) &&
      this.getEnrichmentServicesForType(primaryObject.type).length > 0
    ) {
      try {
        ...
        this.logger.debug(`Enriched primary object ${primaryObject.id}`, {
          ...
          extensions: enrichedObject.extensions
            ? Object.keys(enrichedObject.extensions)
            : [],
        });
      } catch (error) {
        this.logger.warn(
          `Failed to enrich primary object ${primaryObject.id}`,
          {
            error: error.message,
            objectId: primaryObject.id,
            configId: config.id,
    ...
    const enrichedAdditionalObjects = await Promise.all(
      additionalObjects.map(async (obj) => {
        if (
          !nonEnrichableTypes.includes(obj.type) &&
          this.getEnrichmentServicesForType(obj.type).length > 0
        ) {
          try {
            ...
            this.logger.debug(`Enriched additional object ${obj.id}`, {
              objectId: obj.id,
              type: obj.type,
              extensions: enrichedObj.extensions
                ? Object.keys(enrichedObj.extensions)
                : [],
            });
            return enrichedObj;
          } catch (error) {
        ...
        this.logger.debug(
          `Skipped enrichment for additional object ${obj.id} (type: ${obj.type})`,
          {
            objectId: obj.id,
            type: obj.type,
          },
    ...
    const { _alreadyMapped, _sourceConfig, ...cleanEnrichedObject } =
      enrichedObject;
  ...
  private async enrichObject(
    ...
    if (!enrichmentServices.length) {
      this.logger.debug(
        `No enrichment services available for type ${object.type}`,
        {
          objectId: object.id,
          type: object.type,
        },
  ...
  private applyEnrichmentData(
    ...
    Object.entries(enrichmentData).forEach(([serviceKey, data]) => {
      const extensionDef =
        ENRICHMENT_EXTENSIONS[serviceKey as EnrichmentServiceKey];
      if (!extensionDef) {
        this.logger.warn(
          `No extension definition found for enrichment service ${serviceKey}`,
          {
            objectId: object.id,
            type: object.type,
          },
      ...
      this.logger.debug(
        `Added ${serviceKey} enrichment as extension ${extensionDef.id} for object ${object.id}`,
        {
          objectId: object.id,
          type: object.type,
          serviceKey,
  ...
  private async *fetchAndMapStixObjects(
    config: FeedProviderConfig,
    retryCount: number = 0,
  ): AsyncGenerator<GenericStixObject, void, unknown> {
    ...
    while (hasMore) {
      ...
      while (attempt <= maxRetries) {
        try {
          ...
          if (config.name === 'AlienVaultOTX' && responseData?.results)
            responseData = responseData.results;
          if (config.name === 'HybridAnalysis' && responseData?.data)
          ...
          this.logger.log(
            `Fetched page ${page} for ${config.name}: ${dataArray.length} items retrieved, ${mappedCount} STIX objects mapped in ${pageDuration}s`,
            {
              feed: config.name,
              page,
          ...
          if (
            ...
            page >= config.pagination.maxPages
          ) {
            hasMore = false;
          }
          page++;
          ...
          if (error instanceof AxiosError) {
            ...
            this.logger.error(
              `API failed for ${config.name} after ${attempt + 1} attempts`,
              {
                feed: config.name,
                error: message,
              },
            ...
            throw new InternalServerErrorException(
              `${config.name} API failed after ${attempt + 1} attempts: ${message}`,
            );
          } else {
            this.logger.error(
              `Unexpected error in fetchAndMapStixObjects for ${config.name}`,
              {
                error: error.message,
                stack: error.stack,
              },
  ...
  private async createRelationships(
    ...
    const isValidRelationship = (rel: any): boolean => {
      ...
      if (!sourceType || !targetType || !relationshipType) {
        this.logger.warn(
          `Invalid relationship: missing source, target, or type`,
          {
    ...
    const relationships = result.relationships
      ...
      .map((rel: any) => ({
        ...rel,
        source_ref:
          rel.source_ref === result.input.id ? storedObject.id : rel.source_ref,
        object_marking_refs: rel.object_marking_refs || [
          TLP_MARKINGS['white'].id,
        ],
      }));

    if (relationships.length) {
      this.logger.debug(
        `Creating ${relationships.length} valid relationships for ${result.input.type}`,
        {
      ...
      for (let i = 0; i < relationships.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(
          batch.map((rel: any) => this.relationshipService.create(rel)),
        );
        results.forEach((result, index) => {
          if (result.status === 'rejected') {
            this.logger.error(
              `Failed to create relationship ${batch[index].id}`,
              {
      ...
      this.logger.warn(
        `No valid relationships to create for ${result.input.type}`,
        {
          objectId: storedObject.id,
  ...
  private async storeStixObject(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          this.logger?.log(
            `Attempt ${attempt}/3 to store ${type} ${object.id}`,
          );
